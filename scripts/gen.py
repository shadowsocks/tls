s = """0x00,0x00   TLS_NULL_WITH_NULL_NULL     Y   N   [RFC5246]
...
lines = s.split("\n")
for line in lines:
    if "Unassigned" not in line and "Reserved" not in line:
        tmp = line.split(" ")
        code = tmp[0]
        name = None
        for elem in tmp[1:]:
        ...
        print("Self::%s => write!(f, \"%s\")," % (name, name))
        # print("    pub const %s: Self = Self(u16::from_be_bytes([%s]));" %(name, code.replace(",", ", ")) )
